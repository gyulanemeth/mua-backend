...
export default async ({
  ...
  apiServer.patch('/v1/system-admins/:id/email', async req => {
    ...
    const token = jwt.sign(payload, secrets[0], { expiresIn: '24h' })
    const mail = await sendVerifyEmail(req.body.newEmail, process.env.BLUEFOX_TEMPLATE_ADMIN_VERIFY_EMAIL_ID, { link: `${process.env.APP_URL}system-admins/verify-email?token=${token}`, name: response.result.name })
    return {
      status: 200,
      result: {
        success: true,
